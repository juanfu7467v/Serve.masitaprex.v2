...
const BOX_VERTICAL_SPACING = 15;
const AVATAR_SIZE = 80;

const COLOR_TITLE = '#000000';
const COLOR_LINE = '#555555';
...
const drawDataBox = async (ctx, data, index, startX, startY, isWorkApi) => {
    const BOX_RADIUS = 10;
    ctx.fillStyle = '#FFFFFF';
    ctx.beginPath();
    ctx.roundRect(startX, startY, BOX_WIDTH, BOX_HEIGHT, BOX_RADIUS);
    ctx.fill();

    const gradient = ctx.createLinearGradient(startX, startY, startX + BOX_WIDTH, startY + BOX_HEIGHT);
    ...
    // Extraer info según API
    let title = isWorkApi ? data.rz : data.razon_social;
    ...
    const avatarCenterY = startY + BOX_HEIGHT/2;
    ...
    ctx.fillText(title.charAt(0), avatarCenterX, avatarCenterY + 10);
    ...
    ctx.fillText(title, textStartX, startY + 50);

    ctx.fillStyle = COLOR_TEXT;
    ctx.font = `18px ${FONT_FAMILY}`;
    ctx.fillText(subtitle1, textStartX, startY + 85);
    ctx.fillText(subtitle2, textStartX, startY + 115);
    ctx.fillText(subtitle3, textStartX, startY + 145);
    if(extraInfo) ctx.fillText(extraInfo, textStartX, startY + 175);

    return { centerY: startY + BOX_HEIGHT/2 };
};
...
const generateReportImage = async (dni, list, titleText, isWorkApi) => {
    ...
    const boxesHeight = totalBoxes * (BOX_HEIGHT + BOX_VERTICAL_SPACING);
    const CANVAS_HEIGHT = HEADER_HEIGHT + boxesHeight + FOOTER_HEIGHT + (MARGIN * 2);

    const canvas = createCanvas(CANVAS_WIDTH, Math.max(CANVAS_HEIGHT, 600));
    ...
    ctx.textAlign = 'center';
    ctx.fillText(`REPORTE DE ${titleText}`, CANVAS_WIDTH/2, 60);
    ctx.font = `bold 25px ${FONT_FAMILY}`;
    ctx.fillText(`DNI CONSULTADO: ${dni}`, CANVAS_WIDTH/2, 100);

    let currentY = HEADER_HEIGHT;
    const boxStartX = (CANVAS_WIDTH - BOX_WIDTH) / 2;

    for (let i = 0; i < list.length; i++) {
        await drawDataBox(ctx, list[i], i, boxStartX, currentY, isWorkApi);
    ...
    ctx.textAlign = 'left';
    ctx.fillText(`SISTEMA DE CONSULTA ${titleText}`, MARGIN, footerY);
    ctx.textAlign = 'right';
    ctx.fillText(`REGISTROS: ${list.length}`, CANVAS_WIDTH - MARGIN, footerY);
    ...
    try {
        ...
        res.json({
            ...
            result: {
                ...
                coincidences: [{ message: `Reporte de trabajos ${status}`, url: `${API_BASE_URL}/descargar-ficha?url=${encodeURIComponent(url)}` }]
            }
        });
    } catch (e) {
...
app.get("/consultar-empresas", async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            result: {
                quantity: data.result.quantity,
                coincidences: [{ message: `Reporte de empresas ${status}`, url: `${API_BASE_URL}/descargar-ficha?url=${encodeURIComponent(url)}` }]
            }
        });
    } catch (e) {
        res.status(500).json({ message: "error", detail: e.message });
